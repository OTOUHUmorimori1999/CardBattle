...
{
    [SerializeField] PlayerController Pcon; //‚PPƒRƒ“ƒg
    [SerializeField] float posX;

    public void Callblate()
    {
        if (bodyRoot != null)
            posX = bodyRoot.localPosition.x;
        else
            posX = 0;
    }
...
//		}
	...
	protected void MoveAvatar(uint UserID)
	{
        ...
        posX = targetPos.x;
        float moveLimit = 0.08f;
        //‰ñ”ð
        if (posX < -moveLimit)
        {
            Pcon.AvoidMove(true);
        }
        else if (posX > moveLimit)
        {
            Pcon.AvoidMove(false);
        }
        else
        {
            Debug.Log("‚È‚¢‚â‚ñ");
